...
另外可以发现相比顺序读写测试，随机读写测试下flashcache和两种模式的request的性能随着I/O的增大性能的波动更大，这是由于当I/O块增大时，每个随机I/O块覆盖的连续数据块更多，因此对越大的I/O块，更有可能被判断为顺序读写的一部分而将操作定性为不需要缓存的操作，从而将这些I/O操作绕过缓存直接在HDD上执行，因此性能产生比较大的波动。
